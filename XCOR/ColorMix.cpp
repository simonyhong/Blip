...
void CXCOR_View::CosineColorMix(COLORREF RGB_array[],int arraySize) 
{		
	double Ker;
...
void CXCOR_View::CosineColorMix_adjusted(COLORREF RGB_array[],int arraySize)
{
	double Ker;
	double RedKer[1000],GreenKer[1000],BlueKer[1000],shift=0.08;
	double startRED=arraySize*0.75, endRED=arraySize*1;
...
void CXCOR_View::MyOriginalColorMix(COLORREF RGB_array[],int arraySize) 
{	
	int R,G,B;
	double SqDist;
	COLORREF RGB;
...
void CXCOR_View::ColorLegend(CDC *pDC, CFigure *pFig) 
{	
...
void CXCOR_View::FromScrnToDataCoordinate(int x, int y, float &DataX, float &DataY, CFigure *pFig) 
{
