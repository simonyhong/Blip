...
double CXCOR_View::Kernel_2D(double KerSum, double Sigma, double RangeX)
{
...
//It calculates the constant of the Gaussian kernel.
double CXCOR_View::Kernel_1D(double KerSum, double Sigma, double RangeX)
{
	double sigSq=Sigma*Sigma, rangeXSq=RangeX*RangeX, SqDist, Sum; 
...
int CXCOR_View::MakeGssKernel_1D(float KrVal[], int sizeOfKer, int KerFieldX[], double KerSum, double Sigma, double RangeX)
{
...
void CXCOR_View::CreateKer(float sig, float RangeX)
{	
	#define KrCntr 0
	...
	//////////////////////////////////
	m_GaussianAmp=Kernel_1D(1000, sig, RangeX);	//KrSum==1 means no amplification
	NumP=0;
