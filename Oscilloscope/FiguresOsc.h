...
{
public:
	int		WhichChannelSelected(CPoint point);
	double  Divide(double x,double y){double i; i=(y==0)?0:(x/y); return i;}
	void	ListChanIDs(CDC* pDC, int SelectedChan);
	//#######################################
	...
	double m_dChMinX[MAX_AI_CHAN], m_dChMaxX[MAX_AI_CHAN], m_dChMinY[MAX_AI_CHAN], m_dChMaxY[MAX_AI_CHAN];
	...
	void ErasePaneGray(CDC* pm_memDC);
