...
CFigure::CFigure()
{
	...
	m_dLabelBias=0;		m_dLabelScale=1000./(DATA_SAMPLING_RATE); 
...
int CFigure::WhichChannelSelected(CPoint point)
{
	if(point.y<=m_nPaneTy && point.y>=m_nTy ){
		return MAX_AI_CHAN*(Divide(point.x-m_nPaneTx,(m_nPaneBx-m_nPaneTx)));
	}
	else return -1;
}
...
void CFigure::ComputeMapping_XY()
{
	if(m_FontHeight<=0){
	...
	m_Title.StringRect.left=(int)(lX(m_dMinX+m_dSpanX*0.5)-m_FontWidth*m_Title.NameString.GetLength()*0.5);
	m_Title.StringRect.right=(int)(lX(m_dMinX+m_dSpanX*0.5)+m_FontWidth*m_Title.NameString.GetLength()*0.5);
	m_Title.StringRect.top=(int)(m_nTy+m_FontHeight*0.5); m_Title.StringRect.bottom=(int)(m_Title.StringRect.top+m_FontHeight);
	RectOfLabel(0.5,m_Title.NameString.GetLength(),m_Title.StringRect);
}
...
#define Tick 10
	...
	if(m_dGapY<=0){AfxMessageBox("Run ComputeMapping_XY() first.", MB_OK); exit(0);}
...
void CFigure::ListChanIDs(CDC* pDC, int SelectedChan)
{
	char Idx[16];
	float OneDivision=(1.0/MAX_AI_CHAN);
	int left,top;
	for(int i=0;i<MAX_AI_CHAN;i++){
		itoa(i,Idx,10);
		left=lX(((i+0.5)*OneDivision)*m_dSpanX+m_dMinX);
		top=m_Title.StringRect.top-2; 
		if(i==SelectedChan){
			pDC->SetBkColor(RGB(255,190,50));
...
void CFigure::ErasePaneGray(CDC* pm_memDC)
{
	pm_memDC->SelectObject(&m_LightGrayBrush);
	pm_memDC->Rectangle(m_nPaneTx,m_nPaneTy,m_nPaneBx,m_nPaneBy);
}
