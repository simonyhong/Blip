...
public:
	...
	CArray<int, int& >m_SpkTimes[MAX_TRLS_STORED_IN_FIG]; 
	int		m_numSpks[MAX_TRLS_STORED_IN_FIG]; 
	//CTypedPtrList<CObList, CSpksInFig*> m_SpkArrayList; 
	//CSpksInFig	*m_pSpkArrayList[MAX_TRLS_STORED_IN_FIG];
	...
	CArray<double, double& >m_HistOfSpikes; 
	CArray<double, double& >m_SDF; 
	int		m_HistArraySize;
	void	RedZeroAxes(CDC* pDC);
	void	BlackZeroAxes(CDC* pDC);
	void	Build_SpkHistogram_withoutSDF();
	void	Make_SDF_from_Histogram();
	void	CreateKer(double sig, double RangeX);
	void	EmptyHistogramOnly();
	void	EmptyHistogramAndSpikes();
	void	VoidEventMarks();
	void	ReMake_SDF();
	double	KernelAmplitude(double KerSum, double Sigma, double RangeX);
	...
	void	DispSpkRaster_OneType(CDC *pMDC, int isToShowRaster);
	void	DispSDF_OneType(CDC *pMDC, int isToShowSDF);
	...
	ECodeCondition	m_ECodeConditions[256], m_ECodeNullConditions[256], m_OneOfThemMustExist[256];
	int		m_numEodeConditions, m_numEodeNullConditions, m_NumOneOfThemMustExist;
	CString	m_MustExiestECodes,m_MustNotExiestECodes,m_OneOfThemMustExiestECodes;
	void	ExtractECodeConditions(CString ECodeConditionStr, int Mode);
	...
	ClickableLabel m_Title, m_NnRecordingChan, m_Alignment, m_EcodeToMarkLabel;
