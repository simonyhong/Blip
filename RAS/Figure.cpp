...
CFigure::CFigure()
{
	...
	m_ChanID=0;
	m_dSigma=6;
	m_prvUsableTrialIdxInMem=-1;
	m_numSpkArrayLists=0; m_CurrIdxOfSpkTimes=-1; m_OldestSpkArrayIdx=0;
	m_numTimesCalled=0;
	m_Title.m_RelativePos=0.5;
	m_NnRecordingChan.NameString="C:0/N:0"; m_NnRecordingChan.status=1; m_NnRecordingChan.m_RelativePos=0;
	m_Alignment.NameString="Aln: 1"; m_Alignment.NameStringStem="Aln:"; m_Alignment.m_AssociatedVar=0; m_Alignment.status=1; m_Alignment.m_RelativePos=0.7;
	m_EcodeToMarkLabel.NameString="M:";m_EcodeToMarkLabel.NameStringStem="M:"; m_EcodeToMarkLabel.m_AssociatedVar=INT_MAX; m_EcodeToMarkLabel.status=0;  m_EcodeToMarkLabel.m_RelativePos=0.25;
	m_PrvActive=0;
	//##########################////////////
	...
	m_Title.NameString="NoName";m_Title.status=1;
	m_isToShowMarks=0;//Initialized in View at the start of the program
...
CFigure::~CFigure()
{
	m_HistOfSpikes.RemoveAll();
	m_SDF.RemoveAll();
	for(int i=0;i<MAX_TRLS_STORED_IN_FIG;i++){
		m_SpkTimes[i].RemoveAll(); //It deletes the list itself.
	}
...
void CFigure::ExtractECodeConditions(CString ECodeConditionStr, int Mode) 
{	
	int i=0, Num1, Num2, LastSearch=0,charInTmpStr=0, IsRange=0;
	char temp[256];
	CString TmpStr;

	if	   (Mode==ECODE_MUST_EXIEST			){m_numEodeConditions=0;	m_MustExiestECodes=ECodeConditionStr;}
	else if(Mode==ECODE_MUST_NOT_EXIEST		){m_numEodeNullConditions=0;m_MustNotExiestECodes=ECodeConditionStr;}
	else if(Mode==ONE_OF_ECODES_MUST_EXIEST	){m_NumOneOfThemMustExist=0;m_OneOfThemMustExiestECodes=ECodeConditionStr;}
	if(ECodeConditionStr.GetAt(i)=='\0'){return;}
	while(1){
		//Number
		if(ECodeConditionStr.GetAt(i)!=';' && ECodeConditionStr.GetAt(i)!=':' && ECodeConditionStr.GetAt(i)!=' ' && ECodeConditionStr.GetAt(i)!='~'){
			TmpStr+=ECodeConditionStr.GetAt(i); charInTmpStr++;
		}
		//Range character
		else if(ECodeConditionStr.GetAt(i)=='~'){ 
			TmpStr+=ECodeConditionStr.GetAt(i); 
			strcpy(temp,TmpStr);
			if(Find_Last_Num_inString(temp, Num1)==-1){AfxMessageBox("~ is not proceeded by a number"); return;}
			TmpStr=""; 
			charInTmpStr=0;
			IsRange=1;
		}
		//Separator
		else if(charInTmpStr!=0){ 
			TmpStr+=ECodeConditionStr.GetAt(i); 
			FIND_NUMBER_FROM_STR:
			strcpy(temp,TmpStr);
			if(IsRange==0){
				if(Find_Last_Num_inString(temp, Num1)!=-1){
					if(Mode==ECODE_MUST_EXIEST){
						m_ECodeConditions[m_numEodeConditions].m_ECode1=Num1; 
						m_ECodeConditions[m_numEodeConditions++].IsRange=IsRange;
					}
					else if(Mode==ECODE_MUST_NOT_EXIEST){
						m_ECodeNullConditions[m_numEodeNullConditions].m_ECode1=Num1; 
						m_ECodeNullConditions[m_numEodeNullConditions++].IsRange=IsRange;
					}
					else if(Mode==ONE_OF_ECODES_MUST_EXIEST){
						m_OneOfThemMustExist [m_NumOneOfThemMustExist].m_ECode1=Num1; 
						m_OneOfThemMustExist [m_NumOneOfThemMustExist++].IsRange=IsRange;
					}
				}
			}
			else{
				if(Find_Last_Num_inString(temp, Num2)!=-1){
					if(Mode==ECODE_MUST_EXIEST){
						m_ECodeConditions[m_numEodeConditions].m_ECode1=Num1; 
						m_ECodeConditions[m_numEodeConditions].m_ECode2=Num2; 
						m_ECodeConditions[m_numEodeConditions++].IsRange=IsRange;
					}
					else if(Mode==ECODE_MUST_NOT_EXIEST){
						m_ECodeNullConditions[m_numEodeNullConditions].m_ECode1=Num1; 
						m_ECodeNullConditions[m_numEodeNullConditions].m_ECode2=Num2; 
						m_ECodeNullConditions[m_numEodeNullConditions++].IsRange=IsRange;
					}
					else if(Mode==ONE_OF_ECODES_MUST_EXIEST){
						m_OneOfThemMustExist [m_NumOneOfThemMustExist].m_ECode1=Num1; 
						m_OneOfThemMustExist [m_NumOneOfThemMustExist].m_ECode2=Num2; 
						m_OneOfThemMustExist [m_NumOneOfThemMustExist++].IsRange=IsRange;
					}
				}
			}
			if(LastSearch==1)break;
			TmpStr=""; 
			IsRange=0;
			charInTmpStr=0;
		}
		i++;
		if(ECodeConditionStr.GetAt(i)=='\0'){LastSearch=1; goto FIND_NUMBER_FROM_STR;}
	}
}

int CFigure::Find_MUST_and_MustNOT_ECodes(int *pECode, int numEcodes) 
{			
	int i,p, ECodeToLookFor,Num1,Num2;
	//Must haves
	for(i=0;i<m_numEodeConditions;i++){
		if(m_ECodeConditions[i].IsRange==0){
			ECodeToLookFor=m_ECodeConditions[i].m_ECode1;
			for (p = 0; p < numEcodes; p++) {
				if (ECodeToLookFor == pECode[p])break;
			}
		}
		else{
			Num1=m_ECodeConditions[i].m_ECode1;
			Num2=m_ECodeConditions[i].m_ECode2;
			for(p=0;p<numEcodes;p++){
				if(pECode[p]>=Num1 && pECode[p]<=Num2)break;
			}
		}
		if(p==numEcodes)return 0;//There is no such ECode in this trial
	}
	//Must NOT haves
	for(i=0;i<m_numEodeNullConditions;i++){
		if(m_ECodeNullConditions[i].IsRange==0){
			ECodeToLookFor=m_ECodeNullConditions[i].m_ECode1;
			for(p=0;p<numEcodes;p++){
				if(ECodeToLookFor==pECode[p])return 0;
			}
		}
		else{
			Num1=m_ECodeNullConditions[i].m_ECode1;
			Num2=m_ECodeNullConditions[i].m_ECode2;
			for(p=0;p<numEcodes;p++){
				if(pECode[p]>=Num1 && pECode[p]<=Num2)return 0;
			}
		}
	}
...
int CFigure::Find_OneOfThemMustExist_ECodes(int *pECode, int numEcodes) 
{			
	if(m_NumOneOfThemMustExist==0)return 1;

	int i,p, ECodeToLookFor, Num1, Num2;
	for(i=0;i<m_NumOneOfThemMustExist;i++){
		if(m_OneOfThemMustExist[i].IsRange==0){
			ECodeToLookFor=m_OneOfThemMustExist[i].m_ECode1;
			for(p=0;p<numEcodes;p++){
				if(ECodeToLookFor==pECode[p])return 1;
			}
		}
		else{
			Num1=m_OneOfThemMustExist[i].m_ECode1;
			Num2=m_OneOfThemMustExist[i].m_ECode2;
			for(p=0;p<numEcodes;p++){
				if(pECode[p]>=Num1 && pECode[p]<=Num2)return 1;
			}
		}
	}
...
void CFigure::DispRecordingSource(CDC* pobjDC, CDC* pDC) 
{	
	char Source[256];
	sprintf(Source,"C:%d/N:%d",m_ChanID,m_NnID);
	m_NnRecordingChan.NameString=Source;
	pDC->TextOut(m_NnRecordingChan.StringRect.left, m_NnRecordingChan.StringRect.top,m_NnRecordingChan.NameString);
	pobjDC->BitBlt(m_NnRecordingChan.StringRect.left,m_NnRecordingChan.StringRect.top,m_NnRecordingChan.StringRect.right-m_NnRecordingChan.StringRect.left,m_NnRecordingChan.StringRect.bottom-m_NnRecordingChan.StringRect.top,pDC,m_NnRecordingChan.StringRect.left,m_NnRecordingChan.StringRect.top,SRCCOPY);
}
void CFigure::DispLabel(ClickableLabel &Label, CDC* pobjDC, CDC* pDC) 
{	
	char Source[256];
	if(Label.status==1)sprintf(Source,"%s%d",Label.NameStringStem, Label.m_AssociatedVar);
	else               sprintf(Source,"%s"  ,Label.NameStringStem);
	Label.NameString=Source;
	RectOfLabelAlignedToLEFT(Label);
...
void CFigure::Prepare4FigFrame()
{
	...
	m_TrlsPosX=(int)lX(m_dMinX+(0.90)*m_dSpanX);   m_TrlsPosY=(int)(m_nTy+m_FontHeight*0.4); 

	RectOfLabelAlignedAtCENTER(m_Title);
	RectOfLabelAlignedToLEFT(m_NnRecordingChan);
	RectOfLabelAlignedToLEFT(m_Alignment);
	RectOfLabelAlignedToLEFT(m_EcodeToMarkLabel);
}
...
#define Tick 10
	...
	if(m_bDrawFRAME==1){ //Draw the frame and the grid of the pannel
		...
		pDC->TextOut(m_Title.StringRect.left,m_Title.StringRect.top,m_Title.NameString);
		pDC->TextOut(m_NnRecordingChan.StringRect.left,m_NnRecordingChan.StringRect.top,m_NnRecordingChan.NameString);
		pDC->TextOut(m_Alignment.StringRect.left,m_Alignment.StringRect.top,m_Alignment.NameString);
		if(m_EcodeToMarkLabel.status==1)pDC->TextOut(m_EcodeToMarkLabel.StringRect.left,m_EcodeToMarkLabel.StringRect.top,m_EcodeToMarkLabel.NameString);
		else							pDC->TextOut(m_EcodeToMarkLabel.StringRect.left,m_EcodeToMarkLabel.StringRect.top,"M:");
	}
...
int CFigure::Find_Last_Num_inString(char *GivenString, int &NumToReturn)
{
	char *p = NULL;
	char *q = NULL;
...
int CFigure::wildcmp(char *wild, char *string) 
{
