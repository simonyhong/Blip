...
CPen	CFigure::m_PinkPen, CFigure::m_OrangePen, CFigure::m_RedPen, CFigure::m_BluePen, CFigure::m_GreenPen, CFigure::m_DkGreenPen, CFigure:: m_BlackPen1, CFigure:: m_BlackPen2, CFigure::m_BlackPen3, CFigure::m_GrayPen, CFigure::m_LightGrayPen, CFigure::m_Null_Pen;
...
CFigure::CFigure()
{
	...
	m_dGapX=GAP_XnY; m_dGapY=GAP_XnY; // m_dGapY has been set to -1 to warn the user to set the position of the figure first.
	...
	if(m_DiviceContaxtCreatedAlready==0){
		...
		m_BlackPen1.CreatePen(PS_SOLID, 2,RGB(0,0,0));
		...
		m_bkBrush.CreateStockObject(RGB(0,0,0));
...
CFigure::~CFigure()
{
	m_OrangePen.DeleteObject();m_RedPen.DeleteObject();m_PinkPen.DeleteObject();m_BluePen.DeleteObject(); 
	...
	m_bkBrush.DeleteObject();m_LightGrayBrush.DeleteObject();m_GrayBrush.DeleteObject();
}
...
{
	if((Min_X>=Max_X)||(Min_Y>=Max_Y)){AfxMessageBox("(Min_X>=Max_X)||(Min_Y>=Max_Y) ==> Try again", MB_OK); return;}
...
void CFigure::Prepare4FigFrame()
{
	...
	RectOfTopLabel(0.2, m_Point.NameString.GetLength(), m_Point.StringRect);
	RectOfTopLabel(0.7, m_Line.NameString.GetLength(), m_Line.StringRect);
}

void CFigure::RectOfTopLabel(float dataX, int LengthOfLabel, CRect &rect)
{
	rect.left   =(int)lX(m_dMinX+m_dSpanX*dataX)-m_FontWidth*LengthOfLabel*0.5;
	rect.right  =(int)lX(m_dMinX+m_dSpanX*dataX)+m_FontWidth*LengthOfLabel*0.5;
	rect.top    =(int)(m_nTy+m_FontHeight*0.5); 
	rect.bottom = rect.top+m_FontHeight;
}

void CFigure::Disp_Electrode_Position(CDC* pDC)
{
...
void CFigure::RefreshNameBox(CDC* pobjDC, CDC* pm_memDC)
{
	...
	pobjDC->BitBlt(m_Point.StringRect.left,m_Point.StringRect.top,m_Point.StringRect.right-m_Point.StringRect.left,m_Point.StringRect.bottom-m_Point.StringRect.top,	pm_memDC,m_Point.StringRect.left,m_Point.StringRect.top,SRCCOPY);
	pobjDC->BitBlt(m_Line.StringRect.left,m_Line.StringRect.top,m_Line.StringRect.right-m_Line.StringRect.left,m_Line.StringRect.bottom-m_Line.StringRect.top,	pm_memDC,m_Line.StringRect.left,m_Line.StringRect.top,SRCCOPY);
} 		
...
#define Tick 10
	...
	if(m_bDrawFRAME==1){ //Draw the frame and the grid of the pannel
		...
		pDC->TextOut(m_Point.StringRect.left,m_Point.StringRect.top,m_Point.NameString);//Blow-up button
		pDC->TextOut(m_Line.StringRect.left,m_Line.StringRect.top,m_Line.NameString);//Blow-up button
	}
...
void CFigure::DrawGrid(int Mode, CDC* pDC)  
{
...
//	pDC->SelectObject(&m_BlackPen1);
	...
	stepY=m_dSpanY/20.;//Get_VarXY(50,300,gx()/2,480,"Input interval :");
 
	//---------------------------Horizontal lines
   for(L=m_dMinY;L<m_dMaxY;L+= stepY){
	   Lo=m_nBy - (L-m_dMinY)*m_dDy_DoubleGapY_Per_Max_MinY-m_dGapY;
	   pDC->MoveTo((int)(m_nTx+m_dGapX+SHIFT_TO_X)-2,(int)Lo);
	   if(Mode<2)pDC->LineTo((int)(m_nBx-m_dGapX+SHIFT_TO_X+1),(int)Lo);
	   else		 pDC->LineTo((int)(m_nTx+m_dGapX+SHIFT_TO_X)+20,(int)Lo);
   }
   //----------------------------Vertical lines
...
void CFigure::LineAtoB(double X1, double Y1, double X2, double Y2,CDC* pDC)
{
...
void CFigure::FillSolidRect_XY_Centered(double X, double Y,int width, int height, COLORREF rgb ,CDC* pDC)
{ 
	pDC->FillSolidRect(lX(X)-width/2,lY(Y)-height/2,width,height,	rgb);
}

void CFigure::FillSolidRect_XY_Rectangle(double left, double right,double top, double bottom, COLORREF rgb ,CDC* pDC)
{ 
	pDC->FillSolidRect(lX(left),lY(top),lX(right)-lX(left),lY(bottom)-lY(top),	rgb);
}
...
void CFigure::FillSolidRect13x13(double X, double Y, COLORREF rgb ,CDC* pDC)
{  
	pDC->FillSolidRect(lX(X),lY(Y),13,	13,	rgb);
}

void CFigure::FillSolidRect8x8(double X, double Y, COLORREF rgb ,CDC* pDC)
{  
	pDC->FillSolidRect(lX(X)-4,lY(Y)-4,8,	8,	rgb);
}

void CFigure::FillSolidRect7x7(double X, double Y, COLORREF rgb ,CDC* pDC)
{  
...
int CFigure::FromDataToScreenY(double screenY)
{
	return  (lY(screenY)); //They are in (int) form because it is in screen pixel unit
}
