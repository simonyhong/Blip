...
CFigure::CFigure()
{
	...
	m_NnRecordingChan.NameString="Ch:0/Nn:0";
...
void CFigure::ComputeMappingXY()
{
	...
	RectOfTopLabel(0.2, m_NnRecordingChan.NameString.GetLength(), m_NnRecordingChan.StringRect);
}
...
{	
	char Source[256];
	sprintf(Source,"Ch:%d/Nn:%d",m_ChanID,m_NnID);
	...
	if(m_bDrawFRAME==1){ //Draw the frame and the grid of the pannel
		...
		if(m_howManyY<0){ //Default setting (10 horizontal lines)
				...
				if(i%5==0){
					...
					if(m_Is2ndAxis==1){
						if(i==0){
							...
							Y2=(m_dMaxY2-m_dMinY2)*0.5;
						}
						else{
							Y2=m_dMaxY2;
						}
						if(Y2==0||0.0001>fabs(Y2/(int)(Y2)-1)){//The value is either 0, or almost the same (0.01% difference) as an integer
							sprintf(charVarTemp2nd, "%d ", (int)Y2 );
						}
						else{
							sprintf(charVarTemp2nd, "%0.1lf ", (double)Y2 );
						}
			...
			for(i=0;i<m_howManyY;i++){
				...
				if(m_IsLabeledTicY[i]){
					if(m_YTics[i]==0||0.0001>fabs(m_YTics[i]-RoundToClosestInt(m_YTics[i])))sprintf(charVarTemp, "%.0lf ", m_YTics[i] );//The value is either 0, or almost the same (0.01% difference) as an integer
					else sprintf(charVarTemp, "%.1lf", (double)m_YTics[i] );	
					labelPosX=(int)(lX(m_dMinX)-strlen(charVarTemp)*m_FontWidth);
		...
		if(m_howManyX<0){ //Default setting (10 vertical lines)
			...
			for(i=0,L=m_dMinX;L<m_dMaxX+stepX/Defalut10;L+=stepX,i++){
				...
				if(i%2==0){
					if(labelValue==0||0.0001>fabs(labelValue-RoundToClosestInt(labelValue))){//The value is either 0, or almost the same (0.1% difference) as an integer
...
int CFigure::RoundToClosestInt(double Val)
{
	int RoundedUp=(int)(Val+0.5);
...
void CFigure::SpikeTic(double X, double Y, CDC* pDC)
{
	int x=lX(X), y=lY(Y);
	pDC->MoveTo(x,y);
	pDC->LineTo(x,y-5);
...
void CFigure::PutIntValWithPicxelBias(double X, double Y, int pixelBiasY, int NumToPrint, CDC* pDC)
{  
	char strTemp[100];
	sprintf(strTemp,"%d", NumToPrint);     
	pDC->TextOut((int)(lX(X)-3),(int)(lY(Y)-5)+pixelBiasY, strTemp);
}
