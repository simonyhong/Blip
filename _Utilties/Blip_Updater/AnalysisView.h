...
struct FileToReplace{
	int IdForCheck;
	int IsThereParentFolder;
	int CheckedToReplace;
	char ParentFolderName[256];
	char FileName[256];
};

struct FolderToReplace{
	int IdForCheck;
	int IsThereParentFolder;
	int CheckedToReplace;
	char ParentFolderName[256]; //If this is missing, it is assumed to be the "Blip" folder.
	char FolderName[256];
};

struct FileNotToTouch{
	int IdForCheck;
	int IsThereParentFolder;
	int CheckedToReplace;
	char ParentFolderName[256];
...
#define Max_names 100
	FileToReplace m_FileToReplace[Max_names];
	FolderToReplace m_FolderToReplace[Max_names];
	FileNotToTouch m_FileNotToTouch[Max_names];
	int		m_NumFoldersToReplace, m_NumFilesToReplace, m_TotItems;
	int		m_NumFilesNotToTouch;
	void	Find_DeleteFoldersAndFiles(char* searchThisDir, bool searchSubDirs);
	void	Find_CopyFoldersAndFiles(char* searchThisDir, char* DestinationDir);
	int		IsThere_FolderA_inFolderB(char* FolderA, char* inFolderB, int searchChild);
	int		Confirm_Directory();
	int		Confirm_Deletion(char *pSource, char *pDestination);
	int		Get_Check_status(int idx);
	void	Set_Check_status(int idx, int status);
	void	Give_BlipsChildFolderName(char *ChildFolderName);
	void	Give_BlipsChildFolderName_GrandChildFolderName_ToReplace(char *ChildFolderName, char *GrandChildFolderName);
	void	Give_BlipsChild_FileName_ToReplace(char *FileName);
	void	Give_BlipsChildFolderName_FileName_ToReplace(char *ChildFolderName, char *FileName);
	void	Give_BlipsChildFolderName_FileName_NotToTouch(char *ChildFolderName, char *FileName);
	int		CreateFolder_IfNotAlreayThere(char FldrName[]);
	bool	DeleteDirectory(LPCTSTR lpszDir, bool noRecycleBin);
	int CopyDirectory(LPCTSTR Source, LPCTSTR DestFolder);
	//#######################################################################################################################################
    // thread handling
