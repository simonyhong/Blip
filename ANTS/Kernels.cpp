...
double CAntsView::Kernel_2D(double KerSum, double Sigma, double RangeX)
{
...
//It calculates the constant of the Gaussian kernel.
double CAntsView::Kernel_1D(double KerSum, double Sigma, double RangeX)
{
	double sigSq=Sigma*Sigma, rangeXSq=RangeX*RangeX, SqDist, Sum; 
...
int CAntsView::MakeGssKernel_1D(float KrVal[], int sizeOfKer, int KerFieldX[], double KerSum, double Sigma, double RangeX)
{
	...
	CFigure *pBrd=&m_Fig[0][0][0];
	pBrd->Graph2D(KerFieldX[0],KrVal[0],9,&m_DC[m_DisplayedStimLayer]);
	for(i=0; i<NoX; i++){
		pBrd->Graph2D(KerFieldX[i],KrVal[i],2,&m_DC[m_DisplayedStimLayer]);
	}
