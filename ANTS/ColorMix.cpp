...
void CAntsView::LinearColorMix(COLORREF RGB_array[],int arraySize) 
{		
	...
	for(int i=0; i<arraySize; i++){
		m_DC[m_DisplayedStimLayer].SelectObject(m_RED);
...
void CAntsView::LinearColorMix_adjusted(COLORREF RGB_array[],int arraySize) 
{
	int i;
...
void CAntsView::CosineColorMix(COLORREF RGB_array[],int arraySize) 
{		
	double Ker;
...
void CAntsView::MyOriginalColorMix(COLORREF RGB_array[],int arraySize) 
{	
	int R,G,B;
	double SqDist;
	COLORREF RGB;
...
void CAntsView::ColorLegend(CDC *pDC, CFigure *pFig) 
{	
...
#define LeftOrRightOfX 1 //Put the legend on which side of X? (0:left, 1:right)
	...
	for( i=0; i<MaxRGB100; i++){   //Squares
		...
		pFig->FillSolidRect((double)initXdouble,((double)(i+1)/(double)MaxRGB100)*WidthIn100thOfTotalYSpan*pFig->m_dSpanY+pFig->m_dMinY,RecWidth, RecHight,m_RGB100[rgb],pDC);//m_PrevTx10-1 (-1 has been added to compensate the +error in RecWidth)
	}
...
void CAntsView::FromScrnToDataCoordinate(int x, int y, double &DataX, double &DataY, CFigure *pFig) 
{
