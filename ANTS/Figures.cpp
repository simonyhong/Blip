...
CPen CFigure::m_BlackPen1,CFigure::m_BlackPen2,CFigure::m_BlackPen3,CFigure::m_GrayPen,CFigure::m_LightGrayPen,CFigure::m_Null_Pen;
CBrush CFigure::m_bkBrush,CFigure::m_GrayBrush,CFigure::m_LightGrayBrush,CFigure::m_RedBrush;
...
CFigure::CFigure()
{
	...
	m_Captured=0; m_Deleted=0, m_toSave=1;
	m_ClickToSave.NameString="To save  ";
	m_RecordingChan.NameString="RecCh:-1 StmRly:-1"; //m_RecordingChan.NameString stores BOTH the Stimulation & Recording names
	m_StimGatingRelay.NameString="StmRly:-1";
	m_Focus.NameString="O ";
	m_StimGatingRelay.m_ChanID=-1;
	m_LearnedLevel=0;
	m_NumLfpDrawn=0;
	m_IsToDisplay=0;
	m_IsIt_TriggeringRecordingChannel=0;
	
	m_MeasuringLineTime[0]=-8;m_MeasuringLineTime[1]=8;
	for(int i=0;i<NUM_MEASURING_LINES;i++){
		m_MeasuringTimeRect[i]=0;
	}		
	...
	m_dMinX=-10., m_dMaxX=10., m_dMinY=-2., m_dMaxY=2.; m_dSpanX=m_dMaxX-m_dMinX; m_dSpanY=m_dMaxY-m_dMinY;// m_dSpanY has been set to -1 to warn the user to set the "scales" of X, Y axis first
...
CFigure::~CFigure()
{
	m_PinkPen.DeleteObject(); m_OrangePen.DeleteObject();m_RedPen.DeleteObject();m_BluePen.DeleteObject();
	m_GreenPen.DeleteObject();m_BlackPen1.DeleteObject();m_BlackPen2.DeleteObject();m_BlackPen3.DeleteObject();m_GrayPen.DeleteObject();m_LightGrayPen.DeleteObject();m_Null_Pen.DeleteObject();
	m_bkBrush.DeleteObject();m_GrayBrush.DeleteObject();m_LightGrayBrush.DeleteObject();m_RedBrush.DeleteObject();
}

void CFigure::SetLabelsXY(char Xaxis[50], char Yaxis[50])
{
	////////////
	sprintf(m_cXaxis,"%s",Xaxis); // X coordinate name
	sprintf(m_cYaxis,"%s",Yaxis); // Y coordinate name
	////////////
}
...
void CFigure::Compute_Mapping_XY()
{
	if(m_FontHeight<=0){
	...
	RectOfLabel(0.1,m_ClickToSave.NameString.GetLength(),m_ClickToSave.StringRect);
	RectOfLabel(0.5,m_RecordingChan.NameString.GetLength(),m_RecordingChan.StringRect);
	RectOfLabel(0.6,m_StimGatingRelay.NameString.GetLength(),m_StimGatingRelay.StringRect);
	RectOfLabel(0.9,m_Focus.NameString.GetLength(),m_Focus.StringRect);

	for(int i=0;i<NUM_MEASURING_LINES;i++){
		m_MeasuringTimeRect[i].top=m_nPaneTy+m_FontHeight*1.; 
		m_MeasuringTimeRect[i].bottom=m_MeasuringTimeRect[i].top+m_FontHeight*1.; 
	}
}
...
void CFigure::Disp_Rec_Stim_Names(CDC* pDC)
{
	pDC->TextOut(m_RecordingChan.StringRect.left,m_RecordingChan.StringRect.top,m_RecordingChan.NameString);
} 		
void CFigure::RefreshNameBox(CDC* pobjDC, CDC* pm_memDC)
{
	pobjDC->BitBlt(BIT_BLT(m_RecordingChan.StringRect,m_pDC));
} 		

void CFigure::RefreshClickToSaveBox(CDC* pobjDC, CDC* pm_memDC)
{
	pobjDC->BitBlt(BIT_BLT(m_ClickToSave.StringRect,m_pDC));
} 		

void CFigure::DrawMeasuringLines(CPen *pPen)
{	
	CPen *pOldPen;
	char strTemp[100];
	int	i;
	pOldPen=m_pDC->SelectObject(pPen);
	for(i=0;i<NUM_MEASURING_LINES;i++){
		m_pDC->SelectObject(pPen+i);
		m_pDC->MoveTo(m_MeasureingLine_ScrnCoord[i],m_MeasuringTimeRect[i].bottom);m_pDC->LineTo(m_MeasureingLine_ScrnCoord[i],m_nPaneBy);
		sprintf(strTemp,"%0.2lf  ", fabs(m_MeasuringLineTime[i]));     
		m_pDC->TextOut(m_MeasuringTimeRect[i].left,m_MeasuringTimeRect[i].top,strTemp);	
	}
	pOldPen=m_pDC->SelectObject(pPen);
}
...
#define Tick 10
	...
	if(m_dGapY<=0){AfxMessageBox("Run Compute_Mapping_XY() first.", MB_OK); exit(0);}
	...
	if(m_bDrawFRAME==1){ //Draw the frame and the grid of the pannel
		...
		//---------- Names of X,Y axes
		pDC->TextOut((int)(m_nTx+m_dGapX+SHIFT_TO_X+45),(int)(m_nBy-m_dGapY+15),m_cXaxis);//Horizontal coordinate Axis Name
		m_nYlabelPosX=(int)(m_nTx+m_dGapX+SHIFT_TO_X-30);m_nYlabelPosY=(int)(m_nTy+m_dGapY-20);
		pDC->TextOut((int)(m_nTx+m_dGapX+SHIFT_TO_X-30),(int)(m_nTy+m_dGapY-20),m_cYaxis);//Vertical coordinate Axis Name
		//---------- Bold "X=0" line
		...
		//----------- The Clickable Labels here
		pDC->SelectObject(&m_LightGrayPen); 
		pDC->TextOut(m_ClickToSave.StringRect.left,m_ClickToSave.StringRect.top,m_ClickToSave.NameString);
		pDC->TextOut(m_RecordingChan.StringRect.left,m_RecordingChan.StringRect.top,m_RecordingChan.NameString);
		//pDC->TextOut(m_StimGatingRelay.StringRect.left,m_StimGatingRelay.StringRect.top,m_StimGatingRelay.NameString);
		pDC->TextOut(m_Focus.StringRect.left,m_Focus.StringRect.top,m_Focus.NameString);
	}
...
void CFigure::MyLineAtoB(double X1, double Y1, double X2, double Y2)
{
	m_pDC->MoveTo(lX(X1),lY(Y1)); m_pDC->LineTo(lX(X2),lY(Y2));
}

void CFigure::MyMoveTo(double X, double Y, CDC *pObjDC)
{
	pObjDC->MoveTo(lX(X),lY(Y));
}
void CFigure::MyLineTo(double X, double Y, CDC *pObjDC)
{
	pObjDC->LineTo(lX(X),lY(Y));
}
void CFigure::MyLineAtoB(double X1, double Y1, double X2, double Y2, CDC *pObjDC)
{
	pObjDC->MoveTo(lX(X1),lY(Y1)); m_pDC->LineTo(lX(X2),lY(Y2));
}
...
void CFigure::FillSolidRect_ShiftToX(double X, double Y,int width, int height, COLORREF rgb ,CDC* pDC)
{ 
	pDC->FillSolidRect(lX(X)+1,lY(Y),width,height,	rgb);
}
