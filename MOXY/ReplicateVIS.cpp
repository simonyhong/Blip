...
{	
	...
	else if(strcmp(M_StringMsg_fromTAS_toMOXY,"PvisDrawDisk")==0){
		PvisDrawDisk_ReplicationOfVIS();
	}
	...
	if(messageID!=*M_MsgID_fromTAS_toMOXY && (GetTickCount()-m_PrevTicks>1000)){
		m_PrevTicks=GetTickCount();
...
void CMOXY_View::PvisDrawBar_ReplicationOfVIS()
{
	...
	int nSaveDC=m_DC[0].SaveDC();
	CDC *pDC=&m_DC[0];
	CBrush *pOldBrush=pDC->SelectObject(&m_XY_BlackBrush); //This is done JUST to save the old brush. The SelectObject is not an effective one here because the Rectangle drawing procedure uses an RGB values for the brush.
...
void CMOXY_View::PvisDrawDisk_ReplicationOfVIS()
{
	CSingleLock SyncObj_Display(&m_sema); SyncObj_Display.Lock();
	int nSaveDC=m_DC[0].SaveDC();
	CDC *pDC=&m_DC[0];
	CBrush *pOldBrush=pDC->SelectObject(&m_XY_BlackBrush); //This is done JUST to save the old brush. The SelectObject is not an effective one here because the Rectangle drawing procedure uses an RGB values for the brush.
	pDC->SelectObject(&m_NullPen); //This is done JUST to save the old brush. The SelectObject is not an effective one here because the Rectangle drawing procedure uses an RGB values for the brush.
	...
	for(i=0;i<*M_N;i++){
		...
		//saves the screen points of the rectangle in "m_topleftX[idx],m_topleftY[idx],m_width_inPixels[idx],m_height_inPixels[idx]".
		//The rectangle coordinates "m_topleftX[idx],m_topleftY[idx],m_width_inPixels[idx],m_height_inPixels[idx]" will be used to "unvail" the objects by PvisSwitchStim_ReplicationOfVIS().
		if(m_XY_DisplayMode==EYE_MONITOR_MODE)m_pXYFig->Ellipse_SizeAroundFovia_ReportBackTopLeftXY_VisReplica(M_cntrX[idx], M_cntrY[idx],M_width[idx], M_hight[idx], M_rgb[idx],pDC, m_topleftX[idx],m_topleftY[idx],m_width_inPixels[idx],m_height_inPixels[idx]);
		else                                  m_pXYFig->Ellipse_SizeAroundFovia_ReportBackTopLeftXY(           M_cntrX[idx], M_cntrY[idx],M_width[idx], M_hight[idx], M_rgb[idx],pDC, m_topleftX[idx],m_topleftY[idx],m_width_inPixels[idx],m_height_inPixels[idx]);
	}
	m_DC[0].RestoreDC(nSaveDC); 
	SyncObj_Display.Unlock();
}
