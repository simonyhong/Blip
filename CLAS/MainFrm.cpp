...
#define ROOT_PATH "\\_Config_Files\\Window_Position_Files\\Root_CLAS.txt"
...
BOOL CMainFrame::PreCreateWindow(CREATESTRUCT& cs)
{
	...
	if(RootFileOpen(ROOT_PATH)==1){//Is there a valid root file? 
		...
		if(m_Height_in_screen>=1){
			cs.cy = ::GetSystemMetrics(SM_CYSCREEN)*(1./3.); //Specifies the width of the new window.
			cs.cx = ::GetSystemMetrics(SM_CXSCREEN)*(BOTTOM_RIGHT_WINDOW); //Specifies the height of the new window.
			cs.y = 0;//Specifies the x-coordinate of the upper left corner of the new window. Coordinates are relative to the parent window if the new window is a child window; otherwise coordinates are relative to the screen origin.
			cs.x = 0;//    Specifies the y-coordinate of the upper left corner of the new window. Coordinates are relative to the parent window if the new window is a child window; otherwise coordinates are relative to the screen origin.
		}
