...
public:
	...
	double	m_Threshold, m_ScaleY;
	int		m_prvThreshold_onScrn, m_prvScaleY_onScreen, m_IsScaleHeld;
	ClickableLabel   m_AssociatedChan;
	int		m_ChanID;
	CAiChannel *pChan;
	int		m_row,m_clm,m_dpth;
	double	m_cnvrt;
	void	DispOneWaveform(double *pWave, int DataPointsPerWave, int Ecode, double biasToThreshold, CDC *pDC);
	void	RegisterMinMaxYBase();
	//################################################
	...
	double m_dMinY2, m_dMaxY2, m_dSpanY2, m_dTypedInMinBaseY, m_dTypedInMaxBaseY, m_dTypedInHalfBaseY;
	...
	void SetMinMaxY(double Min_Y, double Max_Y);
	void SetMinMaxXY(double Min_X, double Max_X, double Min_Y, double Max_Y);
	void RescaleAxes(double m_dMinimumX, double m_dMaximumX, double m_dMinimumY, double m_dMaximumY);
	void SetFig_Position_in_ClientView(int Tx, int Ty, int Bx, int By);
	void SetFontSize(CFont *pFont);
	void ComputeMapping_X_Y_Y2();
	void Disp_Y_Scale(CDC* pDC);
	...
	void RefreshFromXandSome(double X1, CDC* pObjDC, CDC* pMemDC); 
	void RepaintPane_2D( CDC* pDC); 
	void RefreshPane( CDC* objDC, CDC* pm_memDC);
	void RefreshPane_TRANS( CDC* objDC, CDC* pmTransDC, CDC* pm_memDC);
