...
void CDasView::LinearColorMix(COLORREF RGB_array[],int arraySize) 
{		
...
void CDasView::LinearColorMix_adjusted(COLORREF RGB_array[],int arraySize) 
{
	int i;
...
void CDasView::CosineColorMix(COLORREF RGB_array[],int arraySize) 
{		
	double Ker;
...
void CDasView::MyOriginalColorMix(COLORREF RGB_array[],int arraySize) 
{	
	int R,G,B;
	double SqDist;
	COLORREF RGB;
...
void CDasView::ColorLegend(CDC *pDC, CFigure *pFig) 
{	
...
void CDasView::FromScrnToDataCoordinate(int x, int y, float &DataX, float &DataY, CFigure *pFig) 
{
...
//This maps a screen point (x,y) to a data point (i,j) on the selected graph.
void CDasView::FromScrnToDataCoordinate2(int y, double &DataY2, CFigure *pFig) 
{
	int paneTopX, paneTopY,paneBottomX, paneBottomY;
	pFig->FromDataToScreenY(pFig->m_dMaxY,paneTopY);	pFig->FromDataToScreenY(pFig->m_dMinY,paneBottomY);//Borrowing the 1st scale system to measure the min and max of the pane
	DataY2=(pFig->m_dMaxY2-pFig->m_dMinY2)*(((double)(paneBottomY-y))/(paneBottomY-paneTopY))+pFig->m_dMinY2;
}
