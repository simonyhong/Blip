...
#define ROOT_PATH "\\_Config_Files\\Window_Position_Files\\Root_DAS_cl.txt"
...
BOOL CMainFrame::PreCreateWindow(CREATESTRUCT& cs)
{
	...
	cs.style &= ~FWS_ADDTOTITLE; //Do not show "Untitled-" 
 	cs.lpszName="DAS-client";
 
	#define BOTTOM_RIGHT_WINDOW 0.25
	...
	if(RootFileOpen(ROOT_PATH)==1){//Is there a valid root file? 
		...
		if(m_Height_in_screen>=1){
			cs.cy = ::GetSystemMetrics(SM_CYSCREEN)*(1./3.); //Specifies the height of the new window.
			cs.cx = ::GetSystemMetrics(SM_CXSCREEN)*(BOTTOM_RIGHT_WINDOW); //Specifies the width of the new window.
			cs.y = 0;//((cs.cy * 3) - cs.cy) / 2; //Specifies the y-coordinate of the upper left corner of the new window. Coordinates are relative to the parent window if the new window is a child window; otherwise coordinates are relative to the screen origin.
			cs.x = ::GetSystemMetrics(SM_CXSCREEN)*BOTTOM_RIGHT_WINDOW;//((cs.cx * 3) - cs.cx) / 2; //Specifies the x-coordinate of the upper left corner of the new window. Coordinates are relative to the parent window if the new window is a child window; otherwise coordinates are relative to the screen origin.
		}
