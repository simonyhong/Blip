...
#define ROOT_PATH "\\_Config_Files\\Window_Position_Files\\Root_VIS.txt"
...
BOOL CMainFrame::PreCreateWindow(CREATESTRUCT& cs)
{
	...
	if(RootFileOpen(ROOT_PATH)==1){//Is there a valid root file? 
		...
		if(m_Height_in_screen>=1){
			...
			cs.y = ::GetSystemMetrics(SM_CYSCREEN) /3; 
		...
		//New format (relative values)
			...
			if(cs.cy<80) cs.cy=80; //Set the minimun so that it gets shown even when the user's monitor is mall.
			cs.cx = RoundToClosestInt(::GetSystemMetrics(SM_CXSCREEN)*(m_Width_in_screen)); //Specifies the width of the new window.
			if(cs.cx<80) cs.cx=80; //Set the minimun so that it gets shown even when the user's monitor is mall.
			//Position
