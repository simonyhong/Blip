...
{  
	...
	//Do the following in CVISView::On3DGraph() 
	...
	showPen.DeleteObject(); erasePen.DeleteObject();		
	return 1;
}
...
void CThreeD::Working(CClientDC* showDC, CClientDC* eraseDC)
{
	...
	if(old_c!=0)m_pVISView->m_3D1.EraseFrameBlack(&(m_pVISView->m_DC[0]));
...
void CThreeD::CoordinateAxis(CClientDC* objDC)
{
	...
	m_pVISView->m_DC[0].SelectObject(objPen);
...
//	}
	...
	for(n=0;n<i;n++){         //Working
		...
		if(n==0)continue;
		m_pVISView->m_3D1.Graph2D(AxisPoint[0].x,AxisPoint[0].y,9, &(m_pVISView->m_DC[0]));
		m_pVISView->m_3D1.Graph2D(AxisPoint[n].x,AxisPoint[n].y,2, &(m_pVISView->m_DC[0]));
	}
	for(n=1;n<i;n++){         //Show
...
void CThreeD::Show(CClientDC* objDC)
{
	...
	default:
		for(Idx=0; Idx<c ; Idx++){
			...
			m_pVISView->m_3D1.Graph2D(mon[Idx].point.x,mon[Idx].point.y,5, objDC);
			old_mon[Idx].point=mon[Idx].point;
		}
	}
}
...
int CThreeD::ThreeD3(int gValue, int InitLine, LPVOID objView, int disp)
{
	...
	do{
		if(m_nKey!=VK_SPACE){
			...
			m_pVISView->m_3D1.EraseFrameBlack(&(m_pVISView->m_DC[0]));
			//if(m_nDrawingMode!=NETWORK)CoordinateAxis(&showDC);
			showDC.SelectObject(&showPen);							
			Show3(&showDC);
			c=0;
			...
			m_pVISView->m_3D2.EraseFrameBlack(&(m_pVISView->m_DC[0]));
			//if(m_nDrawingMode!=NETWORK)CoordinateAxis(&showDC);
			Show30(&showDC);
			...
			ObserverRotation();
			m_pVISView->m_DC[0].SelectObject(&PenForActor);							
			m_pVISView->m_3D2.Graph2D((int)(Scl*EgoCtrcCoordt_SrfcPnt[0]/EgoCtrcCoordt_SrfcPnt[1]),(int)(Scl*EgoCtrcCoordt_SrfcPnt[2]/EgoCtrcCoordt_SrfcPnt[1]),3, &(m_pVISView->m_DC[0]));
			m_pVISView->m_3D2.Graph2D((int)(Scl*EgoCtrcCoordt_SrfcPnt[0]/EgoCtrcCoordt_SrfcPnt[1]),(int)(Scl*EgoCtrcCoordt_SrfcPnt[2]/EgoCtrcCoordt_SrfcPnt[1]),9, &(m_pVISView->m_DC[0]));
			// PIover2 has been added because I defined that y axis (_|_)is 0 degree.
			...
			preRot_EgoCtrcCoordt_SrfcPnt[2]=(ObserverCntrZ) -Observer2CntrZ;
			ObserverRotation();
			m_pVISView->m_3D2.Graph2D((int)(Scl*EgoCtrcCoordt_SrfcPnt[0]/EgoCtrcCoordt_SrfcPnt[1]),(int)(Scl*EgoCtrcCoordt_SrfcPnt[2]/EgoCtrcCoordt_SrfcPnt[1]),2, &(m_pVISView->m_DC[0]));
			m_pVISView->m_3D2.RefreshPane(&showDC,&(m_pVISView->m_DC[0]));
			c=0; 
		}
		MY_PEEK_MESSAGE;
	}while(m_nShowMode==GRAPH_3D);

	showPen.DeleteObject(); PenForActor.DeleteObject();
	return 1;
}
...
void CThreeD::Show3(CClientDC* objDC)
{
	...
	default:
		for(Idx=0; Idx<c ; Idx++){
			m_pVISView->m_3D1.Graph2D(MON[Idx].point.x,MON[Idx].point.y,5, &(m_pVISView->m_DC[0]));
			//objDC->SetPixel(MON[Idx].point.x, MON[Idx].point.y ,RGB(255,0,0));
		}
		m_pVISView->m_3D1.RefreshPane(objDC,&(m_pVISView->m_DC[0]));
	}
}
...
void CThreeD::Show30(CClientDC* objDC)
{
	...
	for(Idx=0; Idx<c ; Idx++){
		m_pVISView->m_3D2.Graph2D(MON[Idx].point.x,MON[Idx].point.y,5, &(m_pVISView->m_DC[0]));
	}
}
