...
double CTASDoc::Mean(double *Array, int N)
{
	int i;
	double mean=0;

	if(N==0){if(m_FirstTime0ElementErr){AfxMessageBox("N=0", MB_OK);  m_FirstTime0ElementErr=0;} return 0;}
	for(i=0; i<N; i++){
		mean+=Array[i];
	}
	return mean/N;
...
double CTASDoc::Mean(float *Array, int N)
{
	int i;
	double mean=0;
...
double CTASDoc::Mean(int *Array, int N)
{
	int i;
	double mean=0;
...
double CTASDoc::SD_GivenM(double *Array, int N,double mean)
{
	int i;
	double SD=0;
...
double CTASDoc::SD_GivenM(float *Array, int N,double mean)
{
	int i;
	double SD=0;
...
double CTASDoc::SD_GivenM(int *Array, int N, double mean)
{
	int i;
	double SD=0;
...
double CTASDoc::Mean_OnlyPositive(double *Array, int N)
{
	int i,n=0;
	double mean=0;
...
double CTASDoc::SD_GivenM_OnlyPositive(double *Array, int N,double mean)
{
	int i,n=0;
	double SD=0;
...
void CTASDoc::Gaussian_WhiteNoise_0to1(double noise[], int numSamples) 
{	
	srand(time(0));
...
double CTASDoc::GenHist(double Data[],int dataSize, int Hist[], int binNum, double binWidth) 
{
	int i, idx;
...
void CTASDoc::BubbleSort(int x[], int count)
{
	int i, j, tempID;
...
void CTASDoc::BubbleSort(int x[], int count, int prvOrder[])
{
...
void CTASDoc::Sort_inAscendingOrder(int x[], int count, int OrderX[])
{
