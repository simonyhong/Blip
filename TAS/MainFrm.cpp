...
{
	// TODO: add member initialization m_EventCode here
...
#define ROOT_PATH "\\_Config_Files\\Window_Position_Files\\Root_TAS.txt"
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	//m_imagelist.Create(16, 15, ILC_COLOR32, 12, 4); 
...
BOOL CMainFrame::PreCreateWindow(CREATESTRUCT& cs)
{
	...
	if(RootFileOpen(ROOT_PATH)==1){//Is there a valid root file? 
		...
		if(m_Height_in_screen>=1){
			//Size
			cs.cy = ::GetSystemMetrics(SM_CYSCREEN)*(0.04762); //Specifies the height of the new window.
			cs.cx = ::GetSystemMetrics(SM_CXSCREEN)*(1-TOP_LEFT_WINDOW); //Specifies the width of the new window.
			//Position
			cs.y = ::GetSystemMetrics(SM_CYSCREEN)*(1./3.5);// y of the upper left corner of the new window. Coordinates are relative to the parent window if the new window is a child window; otherwise coordinates are relative to the screen origin.
			cs.x = ::GetSystemMetrics(SM_CXSCREEN)*0.5; //x of the upper left corner of the new window. Coordinates are relative to the parent window if the new window is a child window; otherwise coordinates are relative to the screen origin.
		}
	...
	else{//Use a default configuration
		//Size
		cs.cy = ::GetSystemMetrics(SM_CYSCREEN)*(0.04762); //Specifies the height of the new window.
		cs.cx = ::GetSystemMetrics(SM_CXSCREEN)*(1-TOP_LEFT_WINDOW); //Specifies the width of the new window.
		//Position
